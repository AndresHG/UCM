...
import es.ucm.fdi.tp.Practica5.AtaxxRules;
...
public class Main {
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructObstaclesOption());
	...
	private static Option constructObstaclesOption() {
		...
		String optionInfo = "You must enter the command whit the format : -o b ; where b is the number of obstacles";
	...
	private static void parseObstaclesOptions(CommandLine line) throws ParseException {
		...
		int obstaclesVal = 0;
		String val;
		val = line.getOptionValue("o");
		if(val != null){
			try{
				 obstaclesVal = Integer.parseInt(val);
			}catch(NumberFormatException e){
				throw new ParseException("Unknown Obstacles format");
			}
		}
		numObstacles = obstaclesVal;
	}
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case Ataxx:
			...
			gameFactory = new AtaxxFactory(dimRows,numObstacles);
			}else if(numObstacles != 0) {
				gameFactory = new AtaxxFactory(numObstacles);
			}else{
