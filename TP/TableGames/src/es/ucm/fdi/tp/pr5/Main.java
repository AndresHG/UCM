...
import es.ucm.fdi.tp.pr5.ataxx.AtaxxFactoryExt;
import es.ucm.fdi.tp.pr5.attt.AtttFactoryExt;
import es.ucm.fdi.tp.pr5.connectn.ConnectNFactoryExt;
...
public class Main {
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case ATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols){
				gameFactory = new AtaxxFactoryExt(dimRows, numAtaxxObstacles);
			}else if(numAtaxxObstacles != 0) {
	...
	public static void startGame() {
		...
		case WINDOW:
			...
			if (multiviews) {
				for (Piece p : pieces) {
					gameFactory.
					createSwingView(g, c, p, gameFactory.createRandomPlayer(),
