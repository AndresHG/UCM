...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) != -1) {
		node2 = myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx];
		/// Directory attributes
		if(node2->tipo == 1) {
			return -EISDIR;
		}
...
static int my_mkdir(const char *path, mode_t mode, dev_t device) {
	char modebuf[10];
...
static int my_read (const char *fd_origen, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	//Crear el buffer con el tamo mximo de bloque
	char buffer[BLOCK_SIZE_BYTES];
	// Size el es tama que se va a leer, viene en la llamada
	...
	while(totalRead < bytes2Read){
		...
		//Bloque actual y desplazamiento en el bloque (donde acabamos de leer la vez anterior)
		...
		//Aadir lo leido al desplazamiento del archivo
		offset+=i;
	}
